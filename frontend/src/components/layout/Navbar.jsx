...
export function Navbar({ user, cartCount, wishlistCount = 0, onMobileMenuToggle }) {
    ...
    <header className="sticky top-0 z-40 bg-slate-950/95 backdrop-blur-xl text-white shadow-xl border-b border-slate-800/80">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16 gap-4">
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          ...
          {currentUser ? (
            <Link to="/dashboard">
              <Button variant="primary" size="sm" className="shadow-md shadow-brand-500/20">
                Dashboard
              </Button>
            </Link>
          ) : (
      ...
      <div className="bg-slate-900/90 backdrop-blur-md px-4 sm:px-6 py-2 text-xs text-slate-300 flex items-center justify-between overflow-x-auto border-t border-slate-800/80">
        <div className="flex items-center gap-6 whitespace-nowrap">
          ...
          <Link to="/products?category=Deals" className="text-brand-400 font-bold hover:underline">
            Today's Deals
          </Link>
